    ...
    reducers: {
        ...
        addExpense: (state, action) => {
            if (parseFloat(action.payload) > state.mainBalance) {
                alert(`You can't Spend more than your main balance: ${state.mainBalance}!`);
                return;
            } else {
        ...
        repayLoan: (state, action) => {
            if (state.mainBalance < parseFloat(action.payload)) {
                ...
                alert(`Your Pending Loan is ${state.loanBalance} which is lower than the amount you are paying ${action.payload}!`);
                return;
            } else {
                state.loanBalance -= parseFloat(action.payload);
                state.mainBalance -= parseFloat(action.payload);
...
export const { addIncome, addExpense, getLoan, repayLoan } = accountSlice.actions;
